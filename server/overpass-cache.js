...
async function persistedOverpass(overpassFn, { key, ttlMs, query, timeoutMs }) {
  const existing = getEntry(key);
  if (existing?.fresh) return { value: existing.value, source: 'cache' };
  try {
    ...
    if (existing && Date.now() - existing.created_at < STALE_MAX_AGE_MS) {
      return { value: existing.value, source: 'stale', error: err.message };
    }
    throw err;
  }
}
...
module.exports = { tileBbox, bboxKey, getEntry, setEntry, persistedOverpass, TILE_DEG };
